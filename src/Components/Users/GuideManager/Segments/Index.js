...
export default class Index extends Component {
    ...
    updateStatusSegment = (status) => {
        ...
        this.setState({ Loader: true });
        axios
            .post(APIURL + "guide-manager/update-segment-status", formData, {
                headers: {
                    "Authorization": `Bearer ${token}`
                }
            })
    ...
    segmentRecords = () => {
        ...
        axios
            .post(APIURL + "guide-manager/segment-list", formData, {
                headers: {
                    "Authorization": `Bearer ${token}`
                }
            })
                ...
                <div className='page'>
                                                        ...
                                                        <div className="row">
                                                            ...
                                                            {/* <div className="col-4">
                                                                <div className="col-12  mb-sm-0 mb-lg-0 mb-md-0 mb-xl-0 mb-2">
                                                                    <Input className="field__input" type="text" onChange={(e) => this.handleSearch(e)} placeholder="Search Segment" />
                                                                </div>
                                                            </div> */}
                                                            ...
                                                            <tbody>
                                                                {
                                                                    this.state.segmentList.length > 0 && this.state.segmentList.map((result, index) =>
                                                                        <tr key={index}>
                                                                            ...
                                                                            <td data-label="Status" className="text-start"> {result.status == "active" ? "Active" : "Deactive"}</td>
                                                                            <td data-label="Action" className='text-end'>
                                                                                <Link to={"/guide/edit/segment/" + result.id} color='primary' className='btn btn-sm me-1 btn-success'> <i className="fas fa-pen"></i> </Link>

                                                                                {result.status == "active" ? <Button onClick={this.deleteModal.bind(this, result)} color='danger' className='btn btn-sm btn- btn-width'>Deactivate </Button> : <Button onClick={this.deleteModal.bind(this, result)} color='success' className='btn btn-sm btn- btn-width'>Activate</Button>}
                                                                            </td>
                                                                        </tr>
                                                                    )
                                                                }
                    ...
                    <Modal size="sm" isOpen={this.state.deleteModal} toggle={() => this.closeDeleteModal()}>
                        ...
                        <ModalBody className="border-0 text-center">
                            Are You sure want to {this.state.status === "active" ? "deactivate" : "activate"} ?
                        </ModalBody>
                        <ModalFooter>
                            <div className="d-flex justify-content-end w-100">
                                <Button className="btn btn-success"
                                    onClick={this.state.status === "active" ? this.updateStatusSegment.bind(this, "deactive") : this.updateStatusSegment.bind(this, "active")}
                                > Yes </Button>
